...
Original URL: https://inst.eecs.berkeley.edu/~cs188/textbook/ml/binary.html
Original Path: https://inst.eecs.berkeley.edu/~cs188/textbook/ml/binary.html
...
Mis-classifying x with old w

Updating w

Updated classification of x

Bias

If you tried to implement a perceptron based on what has been mentioned thus far, you will notice one particularly unfriendly quirk. Any decision boundary that you end up drawing will be crossing the origin. Basically, your perceptron can only produce a decision boundary that could be represented by the function w T f ( x ) = 0, where w , f ( x ) \(\in \mathbb{R^n}\). The problem is, even among problems where there is a linear decision boundary that separates the positive and negative classes in the data, that boundary may not go through the origin, and we want to be able to draw those lines.
...
Let’s run one pass through the data with the perceptron algorithm, taking each data point in order. We’ll start with the weight vector
[w_0, w_1, w_2] = [-1, 0, 0]
(where w 0 is the weight for our bias feature, which remember is always 1).
...
\[\mathbf{w}_0 = \begin{bmatrix}-2 & 2 & 1\end{bmatrix}\] \[\mathbf{w}_1 = \begin{bmatrix}0 & 3 & 4\end{bmatrix}\] \[\mathbf{w}_2 = \begin{bmatrix}1 & 4 & -2\end{bmatrix}\]
