...
Original Path: https://scikit-learn.org/stable/modules/clustering.html
...
A demo of K-Means clustering on the handwritten digits data and
...
An example of K-Means++ initialization : Using K-means++
...
MeanShift
clustering aims to discover blobs in a smooth density of
...
Hierarchical clustering with and without structure : Example
...
keeps one row in memory at a time (memory complexity \(\mathcal{O}(n)\) ).
...
if
min_samples=5
and \(x_*\) is the 5th-nearest neighbor of \(x_p\)
...
\(G_{ms}\) defined for a fixed choice of
min_samples
by associating each
...
can set
minimum_cluster_size = min_samples
...
OPTICS is run with the default value of inf set for
max_eps
, then DBSCAN
...
combining reachability distances and data set
ordering_
produces a
...
samples
labels_pred
, the (adjusted or unadjusted) Rand index
...
v_measure_score
is symmetric : it can be used to evaluate
...
TP
( True Positive ): The number of pairs of points that are clustered together
both in the true labels and in the predicted labels.

FP
( False Positive ): The number of pairs of points that are clustered together
in the predicted labels but not in the true labels.

FN
( False Negative ): The number of pairs of points that are clustered together
