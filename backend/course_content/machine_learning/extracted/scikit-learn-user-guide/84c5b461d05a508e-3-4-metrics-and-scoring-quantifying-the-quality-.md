...
Original Path: https://scikit-learn.org/stable/modules/model_evaluation.html
...
evaluation metrics , we want to give some guidance, inspired by statistical
...
Predicting:
Usually, the response variable \(Y\) is a random variable, in the sense that there
is no deterministic function \(Y = g(X)\) of the features \(X\) .
Instead, there is a probability distribution \(F\) of \(Y\) .
...
and [Gneiting2007] , coincide with strictly consistent scoring functions.
...
Note that for regressors, the prediction is done with predict while for
classifiers it is usually predict_proba .

Decision Making:
The most common decisions are done on binary classification tasks, where the result of
predict_proba is turned into a single outcome, e.g., from the predicted
probability of rain a decision is made on how to act (whether to take mitigating
measures like an umbrella or not).
For classifiers, this is what predict returns.
See also Tuning the decision threshold for class prediction .
...
For further criteria on how to select a specific one, see [Fissler2022] .
...
1 The Brier score is just a different name for the squared error in case of
classification with one-hot encoded targets.

2 The zero-one loss is only consistent but not strictly consistent for the mode.
...
T. Gneiting and A. E. Raftery. Strictly Proper
...
T. Fissler, C. Lorentzen and M. Mayer. Model
...
Most commonly this is accuracy for classifiers and the
coefficient of determination ( \(R^2\) ) for regressors.
...
) rely on an internal scoring strategy.
This can be specified using the
scoring
parameter of that tool and is discussed
in the section The scoring parameter: defining model evaluation rules .
...
String name : common metrics can be passed via a string
name.

Callable : more complex metrics can be passed via a custom
metric callable (e.g., function).

Some tools do also accept multiple metric evaluation. See Using multiple metric evaluation
for details.
...
Using custom scorers in functions where n_jobs > 1 -1" title="Link to this dropdown">#
...
The
balanced_accuracy_score
function computes the balanced accuracy , which avoids inflated
...
B. Ray, M. Saeed, A.R. Statnikov, E. Viegas, Design of the 2015 ChaLearn AutoML Challenge , IJCNN 2015.
...
The function
cohen_kappa_score
computes Cohen’s kappa statistic.
...
See Evaluate the performance of a classifier with Confusion Matrix
...
The
hamming_loss
computes the average Hamming loss or Hamming
...
Tsui, Q. Cao, Commonly used software tools produce conflicting and overly-optimistic AUPRC values , Genome Biology 2024.
...
The
jaccard_score
function computes the average of Jaccard similarity
...
n_labels
). If \(w_{i, y_i}\) is the predicted decision for the true label
...
and a probability estimate \(\hat{p} \approx \operatorname{Pr}(y = 1)\) ,
...
Let \(\hat{P}\) be a matrix of probability estimates,
with elements \(\hat{p}_{i,k} \approx \operatorname{Pr}(y_{i,k} = 1)\) .
...
\(\hat{p}_{i,0} = 1 - \hat{p}_{i,1}\) and \(y_{i,0} = 1 - y_{i,1}\) ,
...
confusion_matrix
\(C\) for \(K\) classes. To simplify the
...
value is always +1.
For additional information, see [WikipediaMCC2021] .
...
The
roc_auc_score
function can also be used in multi-class
...
loss can also be computed as \(\text{zero-one loss} = 1 - \text{accuracy}\) .
...
The
brier_score_loss
function computes the Brier score for binary and multiclass
...
Let the true labels for a set of \(N\) data points be encoded as a 1-of-K binary
indicator matrix \(Y\) , i.e., \(y_{i,k} = 1\) if sample \(i\) has
label \(k\) taken from a set of \(K\) labels. Let \(\hat{P}\) be a matrix
of probability estimates with elements \(\hat{p}_{i,k} \approx \operatorname{Pr}(y_{i,k} = 1)\) .
Following the original definition by [Brier1950] , the Brier score is given by:

\[BS(Y, \hat{P}) = \frac{1}{N}\sum_{i=0}^{N-1}\sum_{k=0}^{K-1}(y_{i,k} - \hat{p}_{i,k})^{2}\]

The Brier score lies in the interval \([0, 2]\) and the lower the value the
...
ranges between \([0,1]\) . For binary targets \(y_i \in \{0, 1\}\) and
probability estimates \(\hat{p}_i \approx \operatorname{Pr}(y_i = 1)\)
...
The
class_likelihood_ratios
function computes the positive and negative
likelihood ratios
...
LR+
) is undefined when \(fp=0\) , meaning the
...
also no true positive predictions ( \(tp=0\) ), that the classifier does not predict
...
LR-
) is undefined when \(tn=0\) . Such
divergence is invalid, as \(LR_- > 1.0\) would indicate an increase in the odds of
...
LR+ and LR-
) are undefined when \(tp=fn=0\) , which
...
d2_brier_score
function implements the special case
of D² with the Brier score, see Brier score loss , i.e.:
...
The
r2_score
function computes the coefficient of
determination ,
...
Note that
r2_score
calculates unadjusted \(R^2\) without correcting for
...
The
mean_absolute_error
function computes mean absolute
error , a risk
...
The
mean_squared_error
function computes mean squared
error , a risk
...
The
max_error
function computes the maximum residual error , a metric
...
function computes the mean Tweedie
deviance error
with a
power
parameter ( \(p\) ). This is a metric that elicits
...
mean_pinball_loss
( Pinball loss ). It determines the
quantile level
alpha
