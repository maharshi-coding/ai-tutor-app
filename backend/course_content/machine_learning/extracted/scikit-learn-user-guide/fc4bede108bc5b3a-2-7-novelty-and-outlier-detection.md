...
Original Path: https://scikit-learn.org/stable/modules/outlier_detection.html
...
See Outlier detection on a real data set
...
\(n\) is the number of samples used to build the tree (see [ 1 ]
for more details).
...
differ from that of
fit_predict
. See Novelty detection with Local Outlier Factor .
...
(see Novelty detection with Local Outlier Factor (LOF) ).
