...
Original Path: https://www.programiz.com/python-programming/examples/prime-number
...
to
num - 1
. If we find a factor in that range, the number is not prime, so we set flag to
True
...
range(2,num//2)
or
range(2,math.floor(math.sqrt(num)+1))
...
for...else
statement to do this task without using an additional
flag
variable.
...
clause of the
for
loop runs if and only if we don't break out the
for
