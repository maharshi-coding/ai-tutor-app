...
Original URL: https://www.programiz.com/python-programming/writing-csv-files
Original Path: https://www.programiz.com/python-programming/writing-csv-files
...
Here, we have opened the innovators.csv file in writing mode using
open()
function.
...
- Specifies the
writer
object to write CSV file with quotes around all the entries.

csv.QUOTE_MINIMAL
- Specifies the
writer
object to only quote those fields which contain special characters ( delimiter , quotechar or any characters in lineterminator )
...
A
lineterminator
is a string used to terminate lines produced by
writer
...
It handles how
quotechar
present in the entry themselves are quoted. When
True
, the quoting character is doubled and when
False
, the
escapechar
...
parameter is a string to escape the delimiter if quoting is set to
csv.QUOTE_NONE
and quotechar if doublequote is
False
...
"
in this case) are double-quoted. The entries with
delimiter
