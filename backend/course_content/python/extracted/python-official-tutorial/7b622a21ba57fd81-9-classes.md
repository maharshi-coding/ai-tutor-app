...
assigned to, so you can change the value of
MyClass.i
by assignment.
...
If
x = MyClass()
, as above, this will return the string
'hello world'
.
...
only works if the base class is accessible as
BaseClassName
in the global
scope.)
...
for in
Base1
, then (recursively) in the base classes of
Base1
,
and if it was not found there, it was searched for in
Base2
, and so on.
...
by most Python code: a name prefixed with an underscore (e.g.
_spam
) should
...
__update
identifier since it is replaced with
_Mapping__update
in the
...
are written like regular functions but use the
yield
statement
